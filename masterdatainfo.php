...
//
// Table class for masterdata
//
class cmasterdata extends cTable {
	var $id_barang;
	var $jenis_barang;
	var $jenis_bahan;
	var $kebutuhan;
	...
	function __construct() {
		...
		$this->TableVar = 'masterdata';
		$this->TableName = 'masterdata';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`masterdata`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id_barang
		$this->id_barang = new cField('masterdata', 'masterdata', 'x_id_barang', 'id_barang', '`id_barang`', '`id_barang`', 3, -1, FALSE, '`id_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_barang->Sortable = TRUE; // Allow sort
		$this->id_barang->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_barang'] = &$this->id_barang;

		// jenis_barang
		$this->jenis_barang = new cField('masterdata', 'masterdata', 'x_jenis_barang', 'jenis_barang', '`jenis_barang`', '`jenis_barang`', 200, -1, FALSE, '`jenis_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->jenis_barang->Sortable = TRUE; // Allow sort
		$this->fields['jenis_barang'] = &$this->jenis_barang;

		// jenis_bahan
		$this->jenis_bahan = new cField('masterdata', 'masterdata', 'x_jenis_bahan', 'jenis_bahan', '`jenis_bahan`', '`jenis_bahan`', 200, -1, FALSE, '`jenis_bahan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->jenis_bahan->Sortable = TRUE; // Allow sort
		$this->fields['jenis_bahan'] = &$this->jenis_bahan;

		// kebutuhan
		$this->kebutuhan = new cField('masterdata', 'masterdata', 'x_kebutuhan', 'kebutuhan', '`kebutuhan`', '`kebutuhan`', 200, -1, FALSE, '`kebutuhan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kebutuhan->Sortable = TRUE; // Allow sort
		$this->fields['kebutuhan'] = &$this->kebutuhan;

		// satuan
		$this->satuan = new cField('masterdata', 'masterdata', 'x_satuan', 'satuan', '`satuan`', '`satuan`', 200, -1, FALSE, '`satuan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->satuan->Sortable = TRUE; // Allow sort
		$this->fields['satuan'] = &$this->satuan;

		// harga
		$this->harga = new cField('masterdata', 'masterdata', 'x_harga', 'harga', '`harga`', '`harga`', 200, -1, FALSE, '`harga`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`masterdata`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->id_barang->setDbValue($conn->Insert_ID());
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('id_barang', $rs))
				ew_AddFilter($where, ew_QuotedName('id_barang', $this->DBID) . '=' . ew_QuotedValue($rs['id_barang'], $this->id_barang->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_barang->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@id_barang@", ew_AdjustSql($this->id_barang->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("masterdataview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("masterdataview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "masterdataadd.php?" . $this->UrlParm($parm);
		else
			$url = "masterdataadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("masterdataedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("masterdataadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("masterdatadelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "id_barang:" . ew_VarToJson($this->id_barang->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_barang->CurrentValue)) {
			$sUrl .= "id_barang=" . urlencode($this->id_barang->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["id_barang"]))
				$arKeys[] = ew_StripSlashes($_POST["id_barang"]);
			elseif (isset($_GET["id_barang"]))
				$arKeys[] = ew_StripSlashes($_GET["id_barang"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->kebutuhan->EditValue = $this->kebutuhan->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->id_barang->Exportable) $Doc->ExportCaption($this->id_barang);
					...
					if ($this->kebutuhan->Exportable) $Doc->ExportCaption($this->kebutuhan);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->id_barang->Exportable) $Doc->ExportField($this->id_barang);
						...
						if ($this->kebutuhan->Exportable) $Doc->ExportField($this->kebutuhan);
