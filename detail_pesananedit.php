...
$detail_pesanan_edit = NULL; // Initialize page object first

class cdetail_pesanan_edit extends cdetail_pesanan {
	...
	// Page object name
	var $PageObjName = 'detail_pesanan_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->id_pesanan->CurrentValue == "") {
			$this->Page_Terminate("detail_pesananlist.php"); // Invalid key, return to list
		}
		if ($this->nomor_so->CurrentValue == "") {
			$this->Page_Terminate("detail_pesananlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// detail_barang
			$this->detail_barang->SetDbValueDef($rsnew, $this->detail_barang->CurrentValue, NULL, $this->detail_barang->ReadOnly);

			// jumlah_barang
			$this->jumlah_barang->SetDbValueDef($rsnew, $this->jumlah_barang->CurrentValue, NULL, $this->jumlah_barang->ReadOnly);

			// harga_barang
			$this->harga_barang->SetDbValueDef($rsnew, $this->harga_barang->CurrentValue, NULL, $this->harga_barang->ReadOnly);
...
// Create page object
if (!isset($detail_pesanan_edit)) $detail_pesanan_edit = new cdetail_pesanan_edit();

// Page init
$detail_pesanan_edit->Page_Init();

// Page main
$detail_pesanan_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$detail_pesanan_edit->Page_Render();
?>
...
var CurrentForm = fdetail_pesananedit = new ew_Form("fdetail_pesananedit", "edit");

// Validate form
fdetail_pesananedit.Validate = function() {
...
fdetail_pesananedit.Form_CustomValidate = 
...
fdetail_pesananedit.ValidateRequired = true;
<?php } else { ?>
fdetail_pesananedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$detail_pesanan_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $detail_pesanan_edit->ShowPageHeader(); ?>
<?php
$detail_pesanan_edit->ShowMessage();
?>
<form name="fdetail_pesananedit" id="fdetail_pesananedit" class="<?php echo $detail_pesanan_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($detail_pesanan_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $detail_pesanan_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $detail_pesanan->id_pesanan->EditValue ?></p></span>
</span>
<input type="hidden" data-table="detail_pesanan" data-field="x_id_pesanan" name="x_id_pesanan" id="x_id_pesanan" value="<?php echo ew_HtmlEncode($detail_pesanan->id_pesanan->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $detail_pesanan->nomor_so->EditValue ?></p></span>
</span>
<input type="hidden" data-table="detail_pesanan" data-field="x_nomor_so" name="x_nomor_so" id="x_nomor_so" value="<?php echo ew_HtmlEncode($detail_pesanan->nomor_so->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $detail_pesanan_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fdetail_pesananedit.Init();
</script>
<?php
$detail_pesanan_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$detail_pesanan_edit->Page_Terminate();
?>
