...
class cinventory_list extends cinventory {
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->inventory_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->inventory_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "finventorylistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
		...
		// Initialize
		$sFilterList = "";
		$sFilterList = ew_Concat($sFilterList, $this->inventory_id->AdvancedSearch->ToJSON(), ","); // Field inventory_id
		$sFilterList = ew_Concat($sFilterList, $this->deskripsi_item->AdvancedSearch->ToJSON(), ","); // Field deskripsi_item
		$sFilterList = ew_Concat($sFilterList, $this->kuantitas->AdvancedSearch->ToJSON(), ","); // Field kuantitas
		$sFilterList = ew_Concat($sFilterList, $this->satuan_unit->AdvancedSearch->ToJSON(), ","); // Field satuan_unit
		...
		$sFilterList = ew_Concat($sFilterList, $this->tanggal_masuk->AdvancedSearch->ToJSON(), ","); // Field tanggal_masuk
		if ($this->BasicSearch->Keyword <> "") {
	...
	function RestoreFilterList() {
		...
		$this->deskripsi_item->AdvancedSearch->SearchValue = @$filter["x_deskripsi_item"];
		$this->deskripsi_item->AdvancedSearch->SearchOperator = @$filter["z_deskripsi_item"];
		$this->deskripsi_item->AdvancedSearch->SearchCondition = @$filter["v_deskripsi_item"];
		$this->deskripsi_item->AdvancedSearch->SearchValue2 = @$filter["y_deskripsi_item"];
		$this->deskripsi_item->AdvancedSearch->SearchOperator2 = @$filter["w_deskripsi_item"];
		$this->deskripsi_item->AdvancedSearch->Save();
		...
		// Field satuan_unit
		$this->satuan_unit->AdvancedSearch->SearchValue = @$filter["x_satuan_unit"];
		$this->satuan_unit->AdvancedSearch->SearchOperator = @$filter["z_satuan_unit"];
		$this->satuan_unit->AdvancedSearch->SearchCondition = @$filter["v_satuan_unit"];
		$this->satuan_unit->AdvancedSearch->SearchValue2 = @$filter["y_satuan_unit"];
		$this->satuan_unit->AdvancedSearch->SearchOperator2 = @$filter["w_satuan_unit"];
		$this->satuan_unit->AdvancedSearch->Save();
		...
		// Field tanggal_masuk
		$this->tanggal_masuk->AdvancedSearch->SearchValue = @$filter["x_tanggal_masuk"];
		$this->tanggal_masuk->AdvancedSearch->SearchOperator = @$filter["z_tanggal_masuk"];
		$this->tanggal_masuk->AdvancedSearch->SearchCondition = @$filter["v_tanggal_masuk"];
		$this->tanggal_masuk->AdvancedSearch->SearchValue2 = @$filter["y_tanggal_masuk"];
		$this->tanggal_masuk->AdvancedSearch->SearchOperator2 = @$filter["w_tanggal_masuk"];
		$this->tanggal_masuk->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->deskripsi_item, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->satuan_unit, $arKeywords, $type);
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->deskripsi_item); // deskripsi_item
			$this->UpdateSort($this->kuantitas); // kuantitas
			$this->UpdateSort($this->satuan_unit); // satuan_unit
			$this->UpdateSort($this->jenis); // jenis
			$this->UpdateSort($this->warna); // warna
			$this->UpdateSort($this->tanggal_masuk); // tanggal_masuk
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetsort") {
				...
				$this->deskripsi_item->setSort("");
				$this->kuantitas->setSort("");
				$this->satuan_unit->setSort("");
				$this->jenis->setSort("");
				$this->warna->setSort("");
				$this->tanggal_masuk->setSort("");
			}
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->inventory_id->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"finventorylistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function LoadOldRecord() {
		...
		if (strval($this->getKey("inventory_id")) <> "")
			$this->inventory_id->CurrentValue = $this->getKey("inventory_id"); // inventory_id
		else
			$bValidKey = FALSE;
...
finventorylist.ValidateRequired = true;
<?php } else { ?>
finventorylist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
finventorylist.Lists["x_satuan_unit"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finventorylist.Lists["x_satuan_unit"].Options = <?php echo json_encode($inventory->satuan_unit->Options()) ?>;
finventorylist.Lists["x_jenis"] = {"LinkField":"x_jenis_bahan","Ajax":true,"AutoFill":false,"DisplayFields":["x_jenis_bahan","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"masterdata"};
...
<?php
	...
	if ($bSelectLimit) {
		if ($inventory_list->TotalRecs <= 0)
			$inventory_list->TotalRecs = $inventory->SelectRecordCount();
	} else {
...
<form name="finventorylistsrch" id="finventorylistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
...
<div class="panel panel-default ewGrid inventory">
...
<?php } ?>
<input type="hidden" name="t" value="inventory">
<div id="gmp_inventory" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($inventory_list->TotalRecs > 0 || $inventory->CurrentAction == "gridedit") { ?>
<table id="tbl_inventorylist" class="table ewTable">
<?php echo $inventory->TableCustomInnerHtml ?>
<thead><!-- Table header -->
...
<?php if ($inventory->inventory_id->Visible) { // inventory_id ?>
	<?php if ($inventory->SortUrl($inventory->inventory_id) == "") { ?>
		<th data-name="inventory_id"><div id="elh_inventory_inventory_id" class="inventory_inventory_id"><div class="ewTableHeaderCaption"><?php echo $inventory->inventory_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="inventory_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory->SortUrl($inventory->inventory_id) ?>',1);"><div id="elh_inventory_inventory_id" class="inventory_inventory_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory->inventory_id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory->inventory_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory->inventory_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($inventory->deskripsi_item->Visible) { // deskripsi_item ?>
	<?php if ($inventory->SortUrl($inventory->deskripsi_item) == "") { ?>
		<th data-name="deskripsi_item"><div id="elh_inventory_deskripsi_item" class="inventory_deskripsi_item"><div class="ewTableHeaderCaption"><?php echo $inventory->deskripsi_item->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="deskripsi_item"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory->SortUrl($inventory->deskripsi_item) ?>',1);"><div id="elh_inventory_deskripsi_item" class="inventory_deskripsi_item">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory->deskripsi_item->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory->deskripsi_item->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory->deskripsi_item->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($inventory->kuantitas->Visible) { // kuantitas ?>
	<?php if ($inventory->SortUrl($inventory->kuantitas) == "") { ?>
		<th data-name="kuantitas"><div id="elh_inventory_kuantitas" class="inventory_kuantitas"><div class="ewTableHeaderCaption"><?php echo $inventory->kuantitas->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="kuantitas"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory->SortUrl($inventory->kuantitas) ?>',1);"><div id="elh_inventory_kuantitas" class="inventory_kuantitas">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory->kuantitas->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory->kuantitas->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory->kuantitas->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($inventory->satuan_unit->Visible) { // satuan_unit ?>
	<?php if ($inventory->SortUrl($inventory->satuan_unit) == "") { ?>
		<th data-name="satuan_unit"><div id="elh_inventory_satuan_unit" class="inventory_satuan_unit"><div class="ewTableHeaderCaption"><?php echo $inventory->satuan_unit->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="satuan_unit"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory->SortUrl($inventory->satuan_unit) ?>',1);"><div id="elh_inventory_satuan_unit" class="inventory_satuan_unit">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory->satuan_unit->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory->satuan_unit->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory->satuan_unit->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($inventory->jenis->Visible) { // jenis ?>
	<?php if ($inventory->SortUrl($inventory->jenis) == "") { ?>
		<th data-name="jenis"><div id="elh_inventory_jenis" class="inventory_jenis"><div class="ewTableHeaderCaption"><?php echo $inventory->jenis->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="jenis"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory->SortUrl($inventory->jenis) ?>',1);"><div id="elh_inventory_jenis" class="inventory_jenis">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory->jenis->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory->jenis->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory->jenis->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($inventory->warna->Visible) { // warna ?>
	<?php if ($inventory->SortUrl($inventory->warna) == "") { ?>
		<th data-name="warna"><div id="elh_inventory_warna" class="inventory_warna"><div class="ewTableHeaderCaption"><?php echo $inventory->warna->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="warna"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory->SortUrl($inventory->warna) ?>',1);"><div id="elh_inventory_warna" class="inventory_warna">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory->warna->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory->warna->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory->warna->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($inventory->tanggal_masuk->Visible) { // tanggal_masuk ?>
	<?php if ($inventory->SortUrl($inventory->tanggal_masuk) == "") { ?>
		<th data-name="tanggal_masuk"><div id="elh_inventory_tanggal_masuk" class="inventory_tanggal_masuk"><div class="ewTableHeaderCaption"><?php echo $inventory->tanggal_masuk->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="tanggal_masuk"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory->SortUrl($inventory->tanggal_masuk) ?>',1);"><div id="elh_inventory_tanggal_masuk" class="inventory_tanggal_masuk">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory->tanggal_masuk->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory->tanggal_masuk->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory->tanggal_masuk->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
<span id="el<?php echo $inventory_list->RowCnt ?>_inventory_inventory_id" class="inventory_inventory_id">
<span<?php echo $inventory->inventory_id->ViewAttributes() ?>>
<?php echo $inventory->inventory_id->ListViewValue() ?></span>
</span>
<a id="<?php echo $inventory_list->PageObjName . "_row_" . $inventory_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($inventory->deskripsi_item->Visible) { // deskripsi_item ?>
		<td data-name="deskripsi_item"<?php echo $inventory->deskripsi_item->CellAttributes() ?>>
<span id="el<?php echo $inventory_list->RowCnt ?>_inventory_deskripsi_item" class="inventory_deskripsi_item">
<span<?php echo $inventory->deskripsi_item->ViewAttributes() ?>>
<?php echo $inventory->deskripsi_item->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory->kuantitas->Visible) { // kuantitas ?>
		<td data-name="kuantitas"<?php echo $inventory->kuantitas->CellAttributes() ?>>
<span id="el<?php echo $inventory_list->RowCnt ?>_inventory_kuantitas" class="inventory_kuantitas">
<span<?php echo $inventory->kuantitas->ViewAttributes() ?>>
<?php echo $inventory->kuantitas->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory->satuan_unit->Visible) { // satuan_unit ?>
		<td data-name="satuan_unit"<?php echo $inventory->satuan_unit->CellAttributes() ?>>
<span id="el<?php echo $inventory_list->RowCnt ?>_inventory_satuan_unit" class="inventory_satuan_unit">
<span<?php echo $inventory->satuan_unit->ViewAttributes() ?>>
<?php echo $inventory->satuan_unit->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory->jenis->Visible) { // jenis ?>
		<td data-name="jenis"<?php echo $inventory->jenis->CellAttributes() ?>>
<span id="el<?php echo $inventory_list->RowCnt ?>_inventory_jenis" class="inventory_jenis">
<span<?php echo $inventory->jenis->ViewAttributes() ?>>
<?php echo $inventory->jenis->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory->warna->Visible) { // warna ?>
		<td data-name="warna"<?php echo $inventory->warna->CellAttributes() ?>>
<span id="el<?php echo $inventory_list->RowCnt ?>_inventory_warna" class="inventory_warna">
<span<?php echo $inventory->warna->ViewAttributes() ?>>
<?php echo $inventory->warna->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory->tanggal_masuk->Visible) { // tanggal_masuk ?>
		<td data-name="tanggal_masuk"<?php echo $inventory->tanggal_masuk->CellAttributes() ?>>
<span id="el<?php echo $inventory_list->RowCnt ?>_inventory_tanggal_masuk" class="inventory_tanggal_masuk">
<span<?php echo $inventory->tanggal_masuk->ViewAttributes() ?>>
<?php echo $inventory->tanggal_masuk->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
<?php if (!isset($inventory_list->Pager)) $inventory_list->Pager = new cPrevNextPager($inventory_list->StartRec, $inventory_list->DisplayRecs, $inventory_list->TotalRecs) ?>
